...
if(isset($_GET['pesq'])) {
	if (!empty($_GET['pesq'])) {
			...
			$query = odbc_exec($db, "SELECT
								area.codArea,
								area.descricao,		
								count(assunto.codAssunto) as 'AssuntoArea'
							FROM 
								Area
							LEFT OUTER JOIN
								Assunto
...
if(isset($_GET['dcod'])){
	if(is_numeric($_GET['dcod'])){
		...
		if($numDec > 0){
			$msg = "N&atilde;o foi possivel deletar <br> A area possui dependencia com o(s) campo(s): ";
			...
			if(!odbc_exec($db, "DELETE FROM 
								...
								WHERE 
									codArea =".$_GET['dcod'])){
				$msg = "N&atilde;o foi possivel apagar o dado";
			}else{
...
if(isset($_POST['btnInclude'])) {
	...
	if(odbc_num_rows($verifica) > 0){
		...
		if(!empty($area)){
			...
			$msg .= "N&atilde;o foi possivel inserir. Campo em branco.";
		}
	}
}
...
if(isset($_POST['btnAlterar']  )){
	if(is_numeric($_GET['ecod'])){
		...
		if(!empty($area)){
			...
			$msg .= "N&atilde;o &eacute; possivel atualizar para a descricao para em branco";
		}
	}
}
