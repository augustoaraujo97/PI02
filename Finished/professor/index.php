...
if(isset($_GET['dcod'])){
	if(is_numeric($_GET['dcod'])){
			...
			if(odbc_num_rows($questao) > 0) {
				$msg .= 'Professor em depend&ecirc;ncia em questões';
			}else{
				if(!odbc_exec($db, "DELETE FROM 
										Professor 
									WHERE 
										codProfessor =".$_GET['dcod'])){
					$msg .= "N&atilde;o foi poss&iacute;vel apagar o dado";
				}else{
					header("Location: index.php?dd");
				}
			}
	}else{
		$msg .= "ERRO : ID n&atilde;o v&aacute;lido";
	}
}
// ---------------------------------------- FIM DELETE ---------------------------------------------------------------
// ---------------------------------------- INSERT PROFESSOR ----------------------------------------------------------
...
if(isset($_POST['btnInclude'])) {
	...
	if(!is_numeric($id))@$msg .= "ID inserido n&atilde;o &eacute; numerico <br>";
	if(strlen($id) <> 6)@$msg .= "ID inserido n&atilde;o contem exatos 6 digitos <br>";
	if($tipo <> 'A' && $tipo <> 'P')@$msg .= "Tipo inserido n&atilde;o &eacute; valido <br>";
	...
	if(isset($_POST['btnUpdate'])){
		...
		if(!isset($msg)){
			...
			if($query = odbc_execute($prepare, array($nome,$email,$id,$tipo))){
				...
				$msg .= "N&atilde;o foi possivel atualizar";
			}
