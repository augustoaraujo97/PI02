...
if (isset($_GET['dcod'])) {
	if (is_numeric($_GET['dcod'])) {
		...
		if (!odbc_exec($db, "UPDATE questao SET ativo = 0 WHERE codQuestao = ".$_GET['dcod'])) {
			$msg .= 'N&atilde;o foi poss&iacute;vel desativar o registro N°' . $_GET['dcod'];  
		} else {
	...
	do {
		...
		if(@$alternativa = $_POST['txtAlt'.$i]){
			...
			if (!$query = odbc_execute($alterativainsert, array($i, $alternativa, $correta))) {
				$msg = "N&atilde;o foi possivel inserir a alternativa";
			} else {
				$i++;
			}
		...
		if (isset($msg)) {
			if (isset($_POST['btnUpdate'])) {
				...
				if(!$query = odbc_execute($questao, array($texto, $assunto, 'A', 1, 1, $dificult))){		
					$msg .= "N&atilde;o foi possivel atualiar a quest&atilde;o";
				} 
				...
				do {
					...
					if(@$alternativa = $_POST['txtAlt'.$i]){
						...
						if (!$query = odbc_execute($alterativaatualizar, array($alternativa, $correta))) {
							$msg .= "N&atilde;o foi possivel atualizar a alternativa";
						} else {
							$i++;
						}
			...
			if (isset($ntem)) header("Location: index.php?uc"); else $msg .= 'N&atilde;o foi poss&iacute;vel atualizar';
		}
	}
}
