		...
		if (isset($_GET['ecod'])) {
			...
			if(	substr($_FILES['flArquivo']['type'], 0, 5) == 'image' &&
				...
				if(odbc_execute($stmt, array( 'Teste', $fileParaDB))){
					...
					$teste = odbc_exec($db, 'SELECT * FROM Imagem WHERE codImagem = ' . $inserirImage. '');
					$teste2 = odbc_fetch_array($teste);
					$teste4 = "<img width='100%' src='data:image/jpeg;base64,".base64_encode($teste2['bitmapImagem'])."' />";
					echo $teste4;
